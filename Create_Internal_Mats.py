...
for i in mats:
    ...
    # check material have (Image Texture) node
    if i.node_tree.nodes.get('Image Texture'):
    
        # create internal material
        newOutput = i.node_tree.nodes.new('ShaderNodeOutput')
        newMat = i.node_tree.nodes.new('ShaderNodeMaterial')
        
        # create new links
        newOutputIn = newOutput.input[0]
        newMatOut = newMat.outputs[0]
        newMatIn = newMat.inputs[0]
        ImgOut = i.node_tree.nodes.get('Image Texture').outputs[0]
        i.node_tree.links.new(ImgOut, newMatIn)
